...
if (isset($_POST['action']) && $_POST['action'] == "search" && !empty($_POST['agent_id']) && !empty($_POST['travel_date'])) {
    ...
    $first_bpr = array();
    $first_extar = array();
    $invoices = $recObj->showlist('invoices', $travel_date, $search_island, $search_product, $agent_id, 0);
    if (!empty($invoices)) {
        foreach ($invoices as $invoice) {
            ...
            if (in_array($invoice['id'], $first_booking) == false) {
                ...
                $cot[$invoice['cover_id']][] = !empty($invoice['total_paid']) ? $invoice['total_paid'] : 0;
                ...
                $total[] = $invoice['booktye_id'] == 1 ? ($invoice['booksta_id'] != 2 && $invoice['booksta_id'] != 4) ? ($invoice['bpr_adult'] * $invoice['rate_adult']) + ($invoice['bpr_child'] * $invoice['rate_child']) : $invoice['rate_total'] : $invoice['rate_private'];
                ...
                // $arr_bo[$invoice['id']]['color'] = '00D100';
                // $arr_bo[$invoice['id']]['darken'] = '008000';
            ...
            <tbody>
                ...
                ?>
                        <tr data-dismiss="modal" data-toggle="modal" data-target="#modal-receipt" onclick="modal_receipt(<?php echo $cover_id[$i]; ?>);">
                            ...
                            <td class="text-center"><?php echo !empty($total) ? !empty($extar_total[$cover_id[$i]]) ? number_format(array_sum($total) + array_sum($extar_total[$cover_id[$i]]) - array_sum($cot[$cover_id[$i]])) : number_format(array_sum($total) - array_sum($cot[$cover_id[$i]])) : ''; ?></td>
                        </tr>
                <?php }
                } ?>
            </tbody>
